...
func TestVerifyMessageWithSigProof_Sybil(t *testing.T) {
	...
	sigProofRequest.Query = map[string]interface{}{
		...
		"type":           "KYCAgeCredential",
		"crs":            "3111",
		"gistRoot":       "18845653003614068876832710996329460079999251311121297585472029517224367708999",
	}
	request := CreateAuthorizationRequestWithMessage(reason, "message to sign", verifierID, callbackURL)
	request.Body.Scope = append(request.Body.Scope, sigProofRequest)
		...
		Scope: []protocol.ZeroKnowledgeProofResponse{
			{
				...
				ZKProof: types.ZKProof{
					Proof: &types.ProofData{
						A: []string{
							"11244563728888532162203463065289609446777152808311374677109274612142578268875",
							"11150878552088060674412713768647167795422767285834247694717599113585587583607",
							"1",
						},
						B: [][]string{
							{
								"14080471411497228340239972300514187303972285172125774243094492231943773527047",
								"1815462048360324039776534573909194297991815775956108914773480826989280387685",
							},
							{
								"1187278766172542197798086724971552492917151723578855119825385322425192961490",
								"14878779053303913921624217464805291774720814383555123071029147459889261596474",
							},
							{
								"1",
								"0",
						...
						C: []string{
							"20951253189673667563828755555087467475724241558960784928167709985032195932252",
							"5065511882930096167851550742497510559734594111751692113257681702779810043247",
							"1",
						},
						Protocol: "groth16",
					},
